<template>
  <div class="page-shell page-shell-workspace agent-page">
    ...
    <div class="agent-layout">
      ...
      <main class="agent-chat card"><div class="agent-chat-head"><div class="flex min-w-0 items-center gap-2"><Bot class="h-4 w-4 text-cyan-400" /><div class="min-w-0"><strong class="block truncate">{{ activeTitle }}</strong><span class="agent-status">{{ webSearchEnabled ? '联网资料已开启' : '仅使用本地工具' }}</span></div></div><button class="icon-btn" title="导出会话为 Markdown" :disabled="!messages.length" @click="exportSession"><Download class="h-4 w-4" /></button><button class="icon-btn" title="清空当前会话消息" :disabled="!messages.length || running" @click="clearCurrentSession"><Eraser class="h-4 w-4" /></button></div>
...
import { Activity, ArrowDownToLine, Bot, Brain, Check, Copy, Download, Eraser, Globe2, MessageCircle, MessageSquare, Pencil, Plus, RefreshCw, ScrollText, Search, Send, ShieldAlert, Sparkles, Square, SquarePen, Trash2, UserRound, Wand2 } from 'lucide-vue-next';
