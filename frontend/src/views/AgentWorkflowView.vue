...
<template>
  <div class="page-shell page-shell-workspace agent-page">
    ...
    <div class="agent-layout">
      ...
      <main class="agent-chat card"><div class="agent-chat-head"><div class="flex min-w-0 items-center gap-2"><button class="agent-mobile-session-btn" title="打开会话" @click="mobileSessionsOpen = true"><MessageSquare class="h-4 w-4" />会话</button><Bot class="h-4 w-4 text-cyan-400" /><div class="min-w-0"><strong class="block truncate">{{ activeTitle }}</strong><span class="agent-status">{{ webSearchEnabled ? '联网资料已开启' : '仅使用本地工具' }}</span></div></div><div class="agent-head-menu">
...
const chat = useAgentChat({
  onEventExtra: (event) => {
    if (event.type === 'confirmation_required') appendActivity('需要确认', '请确认这项变更', { status: 'running' });
      ...
      const phaseLabels = {
        loop_started: '开始', loop_iteration: `第 ${event.traceRound || ''} 轮`,
