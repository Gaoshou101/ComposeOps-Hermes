...
- 挂载向导只汇总已纳管项目，生成精确 bind 配置、可选父目录合并方案和重建命令
...
项目发现和现有容器控制只需要 Docker Socket。要编辑配置、拉取 Compose 镜像或创建缺失服务，还需要把目标项目目录以相同路径挂入 ComposeOps：
...
没有挂载的项目仍会显示。目录挂载只解决文件可达性，不会自动授予操作权限；后端还要求项目处于用户明确选择的纳管白名单中。浏览器只提交项目 ID 和固定动作，不能提交任意宿主机路径或命令参数。

服务分散在不同目录时，打开“设置 → 项目纳管”。ComposeOps 会列出自动发现的全部项目，但新项目默认未纳管，不能执行控制、配置、日志、终端或 AI 诊断。勾选需要管理的项目并应用后，只有这些项目进入服务端允许列表；告警和定时镜像检查也只处理已纳管项目。

项目纳管后，即使 Compose 目录尚未挂载，也可以通过 Docker API 启动、重启、停止和查看项目中已经存在的容器。该模式不会创建或删除容器、网络和卷。拉取 Compose 镜像、创建缺失服务和编辑配置仍要求同路径挂载项目目录。

对于已纳管但尚未挂载的项目，页面会生成可复制的 `services.opsdash.volumes` 配置。默认方案逐个挂载精确项目目录；只有多个已选项目处在同一个安全的直接父目录时，界面才会额外显示父目录合并建议，并明确提示权限会扩大。向导不会自动修改宿主机文件或动态创建高权限辅助容器。
