...
import { ArrowDownToLine, Bot, Check, Copy, Eraser, Globe2, MessageCircle, MessageSquare, Pencil, Plus, RefreshCw, ScrollText, Send, ShieldAlert, Sparkles, Square, Trash2, UserRound, Wand2 } from 'lucide-vue-next';
...
const { messages, input, running, sessionId, scrollEl, atBottom, onScroll, scrollBottom, scrollToBottom, nextMessageId, resetSession, sendMessage, approve, reject, interrupt } = chat;
...
/* 回底浮标 */
.agent-scroll-bottom { position: absolute; bottom: 118px; left: 50%; z-index: 15; display: inline-grid; width: 34px; height: 34px; place-items: center; color: #67e8f9; border: 1px solid #155e75; border-radius: 999px; background: rgba(8,47,73,.9); box-shadow: 0 6px 18px rgba(0,0,0,.4); animation: fade-rise .18s ease; }
.agent-scroll-bottom:hover { background: #0e7490; }
@keyframes fade-rise { from { opacity: 0; transform: translate(-50%, 6px); } to { opacity: 1; transform: translate(-50%, 0); } }
.agent-scroll-bottom { transform: translateX(-50%); }
