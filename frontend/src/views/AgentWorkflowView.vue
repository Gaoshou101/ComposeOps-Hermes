...
<template>
  <div class="page-shell page-shell-workspace agent-page">
    ...
    <div class="agent-layout">
      ...
      <main class="agent-chat card"><div class="agent-chat-head"><div class="flex min-w-0 items-center gap-2"><button class="agent-mobile-session-btn" title="打开会话" aria-label="打开会话列表" @click="mobileSessionsOpen = true"><MessageSquare class="h-4 w-4" />会话</button><Bot class="h-4 w-4 text-cyan-400" /><div class="min-w-0"><strong class="block truncate">{{ activeTitle }}</strong><span class="agent-status">{{ webSearchEnabled ? '联网资料已开启' : '仅使用本地工具' }}</span></div></div><div class="agent-head-menu">
...
function appendActivity(label, text, meta = {}) {
  const item = { id: ++activitySeq, label, text: String(text || ''), time: nowTime(), tool: meta.tool || '', durationMs: meta.durationMs ?? null, status: meta.status || '' };
...
function newSession() { resetSession(); resetViewState(); focusInput(); }
...
async function submit() { const text = input.value.trim(); if (!text) return; showLogPicker.value = false; showQuickPrompts.value = false; if (editingMessageId.value) { const targetId = editingMessageId.value; editingMessageId.value = null; try { await editAndResend(targetId, text); } catch (error) { toast.error(error.message); return; } } else { await sendMessage(text, { projectId: projectId.value || undefined, containerId: containerId.value || undefined, webSearchEnabled: webSearchEnabled.value, attachedLogs: attachedLogs.value || undefined }); } await Promise.all([loadSessions(), loadMemories()]); }
...
onDeactivated(() => { mobileSessionsOpen.value = false; resetViewState(); setSubscriberActive(false); });
</script>
