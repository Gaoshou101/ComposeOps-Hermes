...
function handleEvent(event, assistant) {
  if (event.type === 'token' || event.type === 'thought') assistant.content += event.content || '';
  else if (event.type === 'confirmation_required') assistant.confirmation = { ...event, busy: false };
  else if (event.type === 'tool_error' || event.type === 'error') assistant.content += `${assistant.content ? '\n\n' : ''}${event.error || event.content || 'Agent 执行失败'}`;
  ...
  if (event.type === 'tool_result' && event.tool === 'cron.create') window.dispatchEvent(new CustomEvent('composeops:cron-agent-created', { detail: event.result?.result || {} }));
  scrollBottom();
}
