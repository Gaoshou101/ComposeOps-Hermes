...
const data = ref(null); const usage = ref(null); const capabilities = ref({}); const autoRefresh = ref(true); const error = ref(''); const loading = ref(false); const lastUpdated = ref(''); const intervalMs = ref(5000); const histories = ref({}); const trends = ref({ cpu: [], mem: [], net: [] }); let timer;
...
function accumulate() {
  ...
  if (data.value) {
    ...
    trends.value.cpu = trends.value.cpu.slice(-20);
    trends.value.mem = trends.value.mem.slice(-20);
    trends.value.net = trends.value.net.slice(-20);
  }
}
