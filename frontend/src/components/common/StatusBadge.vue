...
const data = computed(() => {
  ...
  if (st === 'healthy') {
    return { label: '健康', dot: 'bg-emerald-400', pulse: true, glow: 'rgba(52, 211, 153, 0.5)', cls: 'text-emerald-300 border-emerald-500/30 bg-emerald-500/10', tip: '服务健康' };
  }
  if (st === 'unhealthy') {
    return { label: '不健康', dot: 'bg-rose-400', pulse: false, glow: 'rgba(251, 113, 133, 0.5)', cls: 'text-rose-300 border-rose-500/40 bg-rose-500/10', tip: '服务不健康,请检查' };
  }
  return STATUS[st] || { label: st || '未知', dot: 'bg-surface-600', pulse: false, glow: '', cls: 'text-surface-400 border-surface-600/40 bg-surface-800/40', tip: '未知状态' };
});
