...
- 多 Compose 文件编辑、YAML 格式化、`docker compose config` 校验与语义校验(depends_on 引用 / 端口冲突 / 缺失镜像)
- 保存前自动备份,保留最近 20 份,可比较(行级 diff 着色)和恢复;保存前变更预览提示将重建/重启的容器
- 容器实时日志:stdout/stderr、搜索、暂停、下载、ERROR/WARN 级别过滤与计数、智能关键字高亮
- 日志级别由后端结构化输出,聚合视图按容器着色;异常退出日志自动留存至事件中心
- 受项目边界约束的 Web Shell(仅 `sh` / `bash`)
- AI 运维诊断:自动附加 Compose 配置和最近 200 行日志、容器只读探针、联网检索;按会话隔离上下文并支持历史回溯
- AI 智能 Agent:自然语言规划 Compose 运维工作流,28 个工具(启停/扩容/配置/环境/网络/卷/安全审计/诊断/告警/定时/维护/指标),风险等级 + 逐步确认,多角色协作,回滚能力,告警实时判定,执行历史/反馈/审计导出可观测
- 服务卡片:状态变更时间戳、Compose 依赖展示、脱敏环境变量预览;总览页健康分数
- 容器 CPU、内存、网络和 Docker 存储用量;趋势跨刷新持久化(localStorage)+ 告警阈值线
- 事件中心:告警持久化(优先级 / 已读 / 静默)、未读计数、WebSocket 实时推送、异常日志展开
- Compose 编辑器:常用模板一键插入(PostgreSQL / Redis / Nginx / 健康检查)
...
勾选 Compose 后，ComposeOps 会按项目创建短生命周期的工作容器，只把该项目目录以相同路径挂入工作容器，并连接 Docker Socket。工作容器默认复用 90 秒，连续切换配置无需重复创建；空闲后自动销毁，因此不需要修改 ComposeOps 自身配置或重建面板。新项目默认两项都不勾选，浏览器也不能提交任意宿主机路径或命令参数。

可通过 `COMPOSEOPS_WORKSPACE_IDLE_MS` 调整空闲保留时间（默认 `90000` 毫秒），通过 `COMPOSEOPS_WORKSPACE_CACHE_MAX` 调整最多缓存的项目数（默认 `8`）。取消项目的 Compose 勾选时，对应工作容器会立即清理。
